...
Все скрипты ожидают, что backend работает на `http://localhost:8160`  
...
1. Убедитесь, что backend-сервисы запущены (см. `TESTING.md`).  
   Swagger UI доступен по адресу `http://localhost:8160/docs` — используйте его, чтобы смотреть схемы запросов/ответов и UUID (группы, факультеты, события и т.д.).

2. Один раз экспортируйте переменные окружения (пример для PowerShell):

   ```powershell
   $env:API_BASE_URL="http://localhost:8160/api/v1"
...
* Swagger (`http://localhost:8160/docs`) — главный источник информации по всем эндпоинтам, структурам ответов, enum-значениям и правилам валидации.
